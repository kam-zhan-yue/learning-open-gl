...
int main() {
  ...
  Shader outlineShader((std::string(SHADER_DIR) + "/vertex.glsl").c_str(), (std::string(SHADER_DIR) + "/outline.glsl").c_str());
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    outlineShader.use();
    float scale = 1.1f;
    drawCubes(cubeVAO, cubeTexture, outlineShader, glm::vec3(1.1));

    glBindVertexArray(0);

    // We need to re-enable writing to the stencil buffer as well as always pass to ensure that the
    // next objects aren't skipped by stencil testing.
    glStencilMask(0xFF);
