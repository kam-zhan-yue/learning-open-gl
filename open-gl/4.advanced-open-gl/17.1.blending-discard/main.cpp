...
void drawGrass(unsigned int VAO, unsigned int texture, Shader &shader, vector<glm::vec3> positions);
...
int main() {
  ...
  Shader blendShader((std::string(SHADER_DIR) + "/vertex.glsl").c_str(), (std::string(SHADER_DIR) + "/blend.glsl").c_str());
  blendShader.use();
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    drawGrass(transparentVAO, grassTexture, blendShader, grassPositions);
